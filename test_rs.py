...
def MCP_nsc(n, edges, k, timeout=1000):
    import multiprocessing
...
def solve_instance(fname):
    """Giải một instance và trả về kết quả dưới dạng dictionary"""
    print(f"\n===== Processing: {fname} =====")
    ...
    for k in range(lb, ub + 1):
        ...
        clique = MCP_nsc(n, edges, k, timeout=1000)
    ...
    result = {
        ...
        'Vertices': n,
        'Edges': len(edges),
        'Max Clique Size': best_k if max_clique else 0,
...
if __name__ == "__main__":
    ...
    # Lấy danh sách tất cả các file .clq.txt trong thư mục instances
    input_files = glob.glob("input/*.clq.txt")
    results = []

    # Chạy từng instance
    for fname in input_files:
        try:
            result = solve_instance(fname)
    ...
    output_file = 'results_nsc_1000.xlsx'
    df.to_excel(output_file, index=False)
    print(f"\nResults saved to {output_file}")
    
    # In bảng tổng kết
    print("\nSummary:")
    print(df[['Instance', 'Vertices', 'Edges', 'Max Clique Size', 'Time (s)']].to_string())
