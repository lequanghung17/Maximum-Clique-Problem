...
def MCP(cnf_file, n, edges, k,timeout=300 ):
    ...
    solver = Glucose3()  # Use Minisat22 for better performance
...
if __name__ == "__main__":
    ...
    for k in range(lb, ub + 1):
        ...
        if clique is not None:
            ...
            print("✗ (dừng lại, k không feasible)", end="  ")
            break  
...
# 1. Degree Upper Bound
# Ý tưởng: Kích thước của clique tối đa không vượt quá (bậc lớn nhất + 1).

# Áp dụng: Nhanh, tính bằng max degree toàn đồ thị.

# 2. Greedy Coloring Upper Bound
# Ý tưởng: Số màu cần để tô màu đồ thị (bằng thuật toán greedy coloring) là một cận trên của clique (clique lớn nhất không vượt quá số màu).

# Áp dụng: Dùng thuật toán tô màu greedy để chia các đỉnh thành các independent set.

# 3. SAT-based Coloring Refinement (Limited k)
# Ý tưởng: Nếu upper bound còn nhỏ, refine bằng cách encode ràng buộc thành SAT rồi kiểm tra với từng k giảm dần (tức là, kiểm tra liệu có clique size k không).

# Áp dụng: Nếu upper bound nhỏ (ví dụ ≤ 100), bạn thử từng k từ cận trên giảm dần, mỗi lần encode MCP thành SAT với cardinality constraint.

# 4. Subgraph Pruning - Top Degree
# Ý tưởng: Thay vì giải trên cả graph lớn, bạn lấy subgraph gồm top các đỉnh có bậc cao nhất (top-k degree) để tối ưu và tính cận trên nhanh hơn.

# Áp dụng: Chỉ lấy ví dụ 150 đỉnh bậc cao nhất để tính upper bound (giảm kích thước không gian tìm kiếm).

# 5. Prune by Degree
# Ý tưởng: Loại bỏ các đỉnh bậc nhỏ hơn (lower bound - 1), vì chúng chắc chắn không thuộc maximum clique.

# Áp dụng: Lọc đỉnh trước khi tính các cận khác.