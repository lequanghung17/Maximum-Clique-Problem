...
def MCP_pbenc_worker(n, edges, k, queue):
    ...
    #card_cnf = PBEnc.equals(lits=xvars, bound=k, encoding=1)  # 0: Sequential # 1: BDD # 2: Network # 3: Bimander
    card_cnf = CardEnc.equals(lits=xvars, bound=k, encoding=8)  # encoding=1: sequential counter
    ...
    for i in range(1, n):
        solver.add_clause([-xvars[i+1], xvars[i]])
...
if __name__ == "__main__":
    ...
    for k in range(lb, ub + 1):
        ...
        clique, timeout = MCP_pbenc(n, edges, k, timeout=300)
        if clique is not None:
            print("✓", end="  ")
            max_clique = clique
            best_k = k
